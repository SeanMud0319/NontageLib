...
public class NontageCommandManager {
    ...
    private static final Map<JavaPlugin, List<Command>> pluginCommands = new HashMap<>();
    ...
    public static void register(JavaPlugin plugin, NontageCommand command) {
        try {
            ...
            pluginCommands.computeIfAbsent(plugin, k -> new ArrayList<>()).add(cmd);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public static void removeCommandByPlugin(JavaPlugin plugin) {
        try {
            CommandMap commandMap = getCommandMap();
            if (!(commandMap instanceof SimpleCommandMap)) return;
